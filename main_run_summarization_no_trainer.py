...
def main():
    ...
    eval_epochs = list(range(int(args.num_train_epochs / 2), args.num_train_epochs, 1))
    result = None
    rouge = None
        ...
        if epoch in eval_epochs:
            ...
            for step, batch in enumerate(eval_dataloader):
                with torch.no_grad():
                    ...
                    # print('decode_pred: ', decoded_preds)
            ...
            if rouge != None:
                result = rouge
                result = {key: {k : round(v * 100, 4)for k, v in value.items()} for key, value in result.items()}
                logger.info(result)
